<?php
	...
	connectDB();
	$result = mysql_query("SELECT hashcode,name,complete,owner,createdt,timestamp FROM $tablename newer 
		WHERE timestamp = (SELECT MAX(timestamp) FROM $tablename WHERE newer.name = name AND showvalue=1 AND complete=1 AND private=0) ORDER BY createdt") or die(mysql_error);
	closeDB();
			...
			if (!empty($result)) 
			{
				...
				while ($row = mysql_fetch_array($result)) 
				{
					echo '
						...
						<td>&nbsp;'.$row['name'].'&nbsp;</td>
						<td>&nbsp;'.$row['owner'].'&nbsp;</td>
						<td>&nbsp;<a href="player.php?puzzle='.$row['hashcode'].'"><img src="images/play.png" title="play puzzle"></img></a><img style="cursor:pointer" onmousedown="sharePuzzle(\''.$row['hashcode'].'\',\''.$row['complete'].'\',\''.$row['name'].'\')" src="images/share.png" title="share puzzle"></img>&nbsp;</td>
						<td>&nbsp;'.$row['createdt'].'&nbsp;</td>
						</tr>';
				}
